...
const getAllClArtworks = async (
  ...
  try { const response = await clevelandApi.get(`/objects/search?page_size=10&images=1&page_size=${itemLimit}&page=${pageNo + 1}${orderBy}${orderSort}${materialStr}${placeStr}${personStr}${searchStr}`);
...
const getAllChicagoArtworks = async (
  ...
  try {
    ...
    console.error("Error fetching Chicago artworks:", error); throw error; }
};
...
const getSingleChicagoArtwork = async (chicagoartwork_id) => {
  ...
  } catch (error) { console.error(`Error fetching Chicago artwork ${chicagoartwork_id}:`, error);
    throw error;
  }
};
